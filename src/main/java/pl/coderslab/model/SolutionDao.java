...
public class SolutionDao {
    ...
    private static final String CREATE_SOLUTION_QUERY =
            "INSERT INTO solutions(created, description, exercise_id, users_id) VALUES (current_timestamp(), ?, ?, ?)";
    private static final String READ_SOLUTION_QUERY =
            "SELECT * FROM solutions where id = ?";
    private static final String UPDATE_SOLUTION_QUERY =
            "UPDATE solutions SET updated = current_timestamp(), description = ?, exercise_id = ?, users_id = ? where id = ?";
    ...
    public Solution create(Solution solution) {
        try (Connection conn = DBUtil.getConnection()) {
            ...
            if (solution.getExercise().getId() > 0) {
                statement.setInt(2, solution.getExercise().getId());
            } else {
    ...
    public Solution[] findAllByExercise(int exerciseId) {
        try (Connection conn = DBUtil.getConnection()) {
            ...
            while (resultSet.next()) {
                ...
                ExerciseDao exerciseDao = new ExerciseDao();
                solution.setExercise(exerciseDao.read(exerciseId2));
                int usersId = resultSet.getInt("user_id");
                UserDao userDao = new UserDao();
                solution.setUser(userDao.read(usersId));
                solutions = addToArray(solution, solutions);
    ...
    public Solution[] getRecent(int number) {
        try (Connection conn = DBUtil.getConnection()) {
            Solution[] solutions = new Solution[0];
